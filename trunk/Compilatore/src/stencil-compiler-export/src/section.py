...
#                self.realComputationDim[index] = self.father.finalSize - 2*self.shape.ordine
    ...
    def buildTree(self):
        ...
        with open(debugFilename,"w") as f:
            ...
            if self.isGood:
                ...
                time1 = time.time()
                f.write("\nESPANSIONE ALBERO\n")
                self.root.expandTree(self.shape.ordine)
                f.write("\nALBERO ESPANSO\n"+str(self.root))
                time2=time.time()
                tempo = time2-time1
                if tempo > 1:
    ...
    def buildCommTree(self):
        ...
        with open(debugFilename,"w") as f:
            if self.isGood:
                ...
                f.write("\nESPANSIONE ALBERO\n")
                self.commTree.expandCommTree(self.shape.ordine,self.father.size)
                f.write("\nALBERO ESPANSO\n"+str(self.commTree))
    ...
    def generaFillSection(self):
        ''' This method generates the code necessary to retrieve data of section self from the scatter buffer
            Suppose: we are in a 2-d domain; shape has order 1
                    section self has tag 01 and dimensions [1,11] therefore it is expanded on the x axis
                    section self is a shift section -> its shift points are: [0,0] [0,9]
            generated code is:
                    for i0=0:0
                        for i1=1:9+extension
                            s01_0[i0][i1] = local[i0+0][i1-1+extension]

            NOTICE: offset affects only the for loops -> it determines which elements of the section buffer must be updated
                    coordinates of the section determine where data from the section is displaced in the data buffer (local in the C code)
        '''
        out = ""
...
#            #GENERATE DISPLACEMENT POINTER -> chiedere a stack overflow?
#            out += ("\t"+config.DATATYPE+ " disp"+str(self.generaId()))
#            for index,item in enumerate(self.sendDim):
#                if item > self.father.ordine:
#                    out += ("["+str(item)+"+extension]")
#                else:
#                    out += ("["+str(item)+"]")
#            out += ";\n"
#
#            #ASSIGN IT TO CORRECT LOCATION ON LOCAL
#            out+="disp"+self.generaId()+"=(int**) &(local"
#            for dimension in self.computationDim:
#                out += "["+str(dimension)+"]"
#            out+=");\n"
            ...
            #GENERATE indexes for the partition matrix es: local[i0+1][i1-1+extension]
            # it is important to use SEND coordinates in order to cancel out the offset (shift case)
            for index,dimension in enumerate(self.sendCoordinates):
                if dimension >= 0 :
                    displ = "+"+str(dimension)
                else:
                    displ = str(dimension)

                #if the displacement is bigger than ordine I have to consider the extension
                if abs(self.computationCoordinates[index]) > self.father.ordine:
                    if self.computationCoordinates[index] > 0:
                        displ += "+extension"
                    else:
                        displ+= "-extension"

                out +="[i"+str(index)+displ+"]"
    ...
    def generaCondensa(self):
        ''' This method generates the code necessary to collect computed data from the buffer of section self and
            put it back on the partition buffer (LOCAL).
            This method is dual of generaFillSections(self)

            NOTICE: It seems like extension can be added ONLY as a positive offset -> check out if this makes sense
        '''
        out = ""
        if self.isGood or self.isLocal:
            ...
            for index,dimension in enumerate(self.computationDim):
                ...
                if dimension > self.father.ordine:
                    extension = "+extension"
                out += ("for (i"+str(index)+" = "+str(offset[index])+" ; i"+str(index)+" <" +str(self.computationDim[index])+extension+";i"+str(index)+"++){\n")

            out += "local"
            for index in range(len(self.computationDim)):
                extension = ""
                if self.computationCoordinates[index] > self.father.ordine:
                    extension = "+extension"
...
#            return False
    ...
    def generaDebugPrint(self,subscript):
        ''' This method generates the DEBUG print for the section self
            it prints out only the buffer specified by subscript
            FIX - print also outside sections?

            subscript   - either 0 or 1 ,specify which buffer has to be output
        '''
        out = ""
        if self.isGood:
            out+= "#if DEBUG\n"
            ...
            for index,item in enumerate(self.sendDim):
                if item > self.father.ordine:
                    ...
                    extension = ""
                out+="for (i"+str(index)+"=0;i"+str(index)+"<"+str(item)+extension+";i"+str(index)+"++){\n"
