...
class Node(object):
    ...
    def expandTree(self,ordine):
        ''' this method is used to expand from the size (defined by the compiler)
            to the size requested by the user. In order to do so, the start and end of a node
            are modified in order to contain a placeholder for code generation phase
